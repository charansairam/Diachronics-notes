...
function createBasesCodeblockTransform(opts: BasesPageOptions | undefined): TreeTransform {
  ...
  return (root: HtmlRoot, _slug: FullSlug, componentData: QuartzComponentProps) => {
    ...
    visit(root, "element", (node: Element, index, parent) => {
      ...
      const transcludeUrl = node.properties?.dataUrl as string | undefined;
      if (node.tagName === "blockquote" && classNames.includes("transclude") && transcludeUrl) {
        ...
        if (matchedBase) {
          ...
          const viewName = block.startsWith("#") ? block.slice(1) : undefined;
          renderIntoNode(node, matchedBase.basesData, viewName, matchedBase.selfContext);
          return;
        }
      }
...
function buildRuntimeBaseLookup(
  allFiles: Record<string, unknown>[],
): Map<string, { basesData: BasesData; selfContext?: EvalContext["self"] }> {
  const lookup = new Map<string, { basesData: BasesData; selfContext?: EvalContext["self"] }>();

  for (const entry of allFiles) {
    const basesData = entry.basesData as BasesData | undefined;
    const slug = typeof entry.slug === "string" ? entry.slug : undefined;
    if (!basesData || !slug) continue;

    const payload = {
      basesData,
      selfContext: entry.basesSelfContext as EvalContext["self"] | undefined,
    };
